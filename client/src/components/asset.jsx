...
class asset extends Component {
  ...
  componentDidMount() {
    ...
    //  showLoader();
    axios
      .get("http://localhost:6007/asset/getAssetInformation")
      .then(response => {
        //console.log(response);
        questions_from_db = response.data.assetLocalData;
  ...
  onSubmit1(e) {
    ...
    //showLoader();
    axios
      .post("http://localhost:6007/asset/addAssetInformation", userData)
      .then(response => {
        //  hideLoader();
        console.log(response);
      })
  ...
  onSubmit() {
    ...
    $("#undo_redo_to > option").each(function() {
      assetQuestiondescription[index] = $(this).text();
      assetQuestionId[index] = $(this).val();
      index = index + 1;
      console.log(assetQuestiondescription);
      console.log(assetQuestionId);
    });

    for (let i = 0; i < assetQuestiondescription.length; i++) {
      debugger;
      const userData = {
        cat_id: cat_id,
        q_desc: assetQuestiondescription[i],
        q_Id: assetQuestionId[i]
      };
      axios
        .post("http://localhost:6007/savedAsset/savedAsset", userData)
        .then(response => {
          console.log(response);
          //    hideLoader();
  ...
  render() {
    ...
    return (
      <div style={{ paddingTop: "5%" }} className="container">
        ...
        <h2 style={{ color: "white", fontSize: "30px" }}>Assets Information</h2>
