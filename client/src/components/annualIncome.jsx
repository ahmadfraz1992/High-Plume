...
var annualIncomeQuestiondescription = [];
var annualIncomeQuestionId = [];
var data;
var cat_id;
function showLoader() {
  $(".overlay").show();
...
class annualIncome extends Component {
  ...
  componentDidMount() {
    ...
    //  showLoader();
    axios
      .get("http://localhost:6007/annualIncome/getannualIncomeInformation")
      .then(response => {
        //console.log(response);
        questions_from_db = response.data.annualIncomeLocalData;
  ...
  onSubmit1(e) {
    ...
    axios
      .post(
        "http://localhost:6007/annualIncome/addannualIncomeInformation",
        userData
      )
      .then(response => {
        //  hideLoader();
    ...
    this.props.history.push(`/annualIncome`);
  }
  ...
  onSubmit() {
    ...
    $("#undo_redo_to > option").each(function() {
      annualIncomeQuestiondescription[index] = $(this).text();
      annualIncomeQuestionId[index] = $(this).val();
      index = index + 1;
      console.log(annualIncomeQuestiondescription);
      console.log(annualIncomeQuestionId);
    });

    for (let i = 0; i < annualIncomeQuestiondescription.length; i++) {
      debugger;
      const userData = {
        cat_id: cat_id,
        q_desc: annualIncomeQuestiondescription[i],
        q_Id: annualIncomeQuestionId[i]
      };
      axios
        .post(
          "http://localhost:6007/savedAnnualIncome/savedAnnualIncome",
          userData
        )
        .then(response => {
          console.log(response);
          //    hideLoader();
          this.props.history.push(`/annualExpendituress`);
        })
        .catch(error => {
          console.log(error.response);
        });
  ...
  render() {
    ...
    return (
      <div style={{ paddingTop: "5%" }} className="container">
        ...
        <h1 style={{ fontSize: "55px" }}>Annual Income (A)</h1>
